...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const submit = async () => {
    if (!name.trim() || !content.trim()) { setError(t('tpl_name_required')); return }
    setSaving(true)
    try {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 w-full max-w-lg space-y-4" onClick={e => e.stopPropagation()}>
        <h2 className="text-xl font-bold text-white">{t('tpl_modal_title')}</h2>
        ...
        <div>
          <textarea
            ...
            placeholder={t('tpl_content_ph')}
            rows={5}
            className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white resize-none"
          />
          {vars.length > 0 && (
            <p className="text-xs text-blue-400 mt-1">
              {t('tpl_vars_detected')}{vars.map(v => <span key={v} className="mx-1 bg-blue-900/50 px-1.5 py-0.5 rounded">{'{' + v + '}'}</span>)}
            </p>
          )}
        </div>
        ...
        <div className="flex gap-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-white">{t('tpl_cancel')}</button>
          <button
            onClick={submit}
            disabled={saving}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 text-white rounded-lg"
          >
            {saving ? t('tpl_creating') : t('tpl_create')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">{t('tpl_title')}</h1>
        <button
          onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm"
        >
          <Plus className="w-4 h-4" /> {t('tpl_create_btn')}
        </button>
      </div>

      {loading ? (
        <p className="text-gray-400">{t('tpl_loading')}</p>
      ) : templates.length === 0 ? (
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {tpl.variables && tpl.variables.length > 0 && (
                    <span className="text-xs text-blue-400 bg-blue-900/30 px-2 py-0.5 rounded">
                      {tpl.variables.length} {t('tpl_var_count')}
                    </span>
                  )}
                </div>
