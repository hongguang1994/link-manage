    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400">
          ...
          {t('dash_online')} {connected} / {total}
        </div>
      </div>

      <div className="grid grid-cols-3 gap-4">
        {[
          { label: t('dash_total'), value: total, color: 'text-blue-400' },
          { label: t('dash_online'), value: connected, color: 'text-green-400' },
          { label: t('dash_offline'), value: total - connected, color: 'text-gray-400' },
        ].map(s => (
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-200 mb-3">{t('dash_device_list')}</h2>
        {loading ? (
